...
import com.wiser.animationlistdemo.combin.bean.ChildrenOpers;
...
public class DiscoverIndexUtil {
    ...
    private static boolean isFilterNullOper(DiscoverOper discoverOper) {
        ...
        return CollectionUtil.isEmpty(childrenOpers.getContainer()) && (CollectionUtil.isEmpty(childrenOpers.getBanner()));
    }


    public static List<DiscoverOper> getOperLevel2ListByParentId(List<DiscoverOper> opers, int parentId) {
        if (opers == null)
            return null;

        List<DiscoverOper> merageOpers = new ArrayList<DiscoverOper>();

        for (DiscoverOper oper : opers) {
            if (oper.getElement_id() == parentId) {
                ...
                List<DiscoverOper> operWebViews = childrenOpers.getContainer();     // WebView
                if (operWebViews == null) {
                    continue;
                }
                for (int j = 0; j < CollectionUtil.size(operWebViews); j++) {

                    DiscoverOper containerOper = operWebViews.get(j);
                    containerOper.setParentTitle(oper.getTitle());
                    containerOper.setChildPosition(j);
                    containerOper.setParentId(oper.getElement_id());
                    merageOpers.add(containerOper);
                }
            }
        }
        return merageOpers;
    ...
    public static List<DiscoverOper> merageOperLevel2List(List<DiscoverOper> opers) {
        ...
        for (int i = 0; i < CollectionUtil.size(opers); i++) {
            ...
            List<DiscoverOper> operWebViews = childrenOpers.getContainer();     // WebView
            if (operWebViews == null) {
                Log.d("ssedss", "merageOperLevel2List: " + i);
                continue;
            }
            // 模拟数据
//            operWebViews.addAll(childrenOpers.getContainer());
//            operWebViews.addAll(childrenOpers.getContainer());
            for (int j = 0; j < CollectionUtil.size(operWebViews); j++) {
