...
public class ScrollingUtil {
    ...
    public static boolean isViewToTop(View view, int mTouchSlop) {
        ...
        boolean isViewToTop = view != null && Math.abs(view.getScrollY()) <= 2 * mTouchSlop;
        Log.e(TAG, "isViewToTop: " + isViewToTop);
        return isViewToTop;
    }

    public static boolean isViewToBottom(View view, int mTouchSlop) {
        boolean isViewBottom = false;
        if (view instanceof AbsListView) isViewBottom = isAbsListViewToBottom((AbsListView) view);
        if (view instanceof RecyclerView)
            isViewBottom = isRecyclerViewToBottom((RecyclerView) view);
        if (view instanceof WebView) isViewBottom = isWebViewToBottom((WebView) view, mTouchSlop);
        if (view instanceof ViewGroup) isViewBottom = isViewGroupToBottom((ViewGroup) view);
        Log.e(TAG, "isViewBottom: " + isViewBottom);
        return isViewBottom;
    }
    ...
    public static boolean isScrollViewToBottom(NestedScrollView scrollView) {
        if (scrollView != null) {
            int scrollContentHeight = scrollView.getScrollY() + scrollView.getMeasuredHeight() - scrollView.getPaddingTop() - scrollView.getPaddingBottom();
            int realContentHeight = scrollView.getChildAt(0).getMeasuredHeight();
            if (scrollContentHeight == realContentHeight) {
